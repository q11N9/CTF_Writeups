...
def tim_a(p, q, t):
    ...
    if q < 2:
        return res
    for i in range(2, q):
        d = nhanBinhPhuong(i,p,q)
        if checkPrime(d, t):
            res.append(i)
    return res
if __name__ == "__main__":
    t = int(input("Nhap t(Danh cho kiem tra Miller-Rabin): "))
    p = generate_Prime(t)
    q = generate_Prime(t)
    while q == p:
        q = generate_Prime(t)
    print(f"2 so nguyen to p, q duoc sinh ra la: {[p, q]}")
    res = tim_a(p, q)
    print(f"Cac so 0 < a < 100 thoa man a^{p} mod {q} la mot so nguyen to la: {res}")
