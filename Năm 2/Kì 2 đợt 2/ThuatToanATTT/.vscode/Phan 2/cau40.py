...
def gcd(a, b):
    while b > 0:
        ...
        b = r
    return a
def gcd_prime_pairs(arr):
    res = []
    l = len(arr)
    for i in range(0, l - 1):
        for j in range(i + 1, l):
            if checkPrime(gcd(arr[i], arr[j])):
...
if __name__ == "__main__":
    ...
    #     x = int(input(f"a[{i}] = "))
    #     arr.append(x)
    gcd_primes = gcd_prime_pairs(arr)
    if len(gcd_primes) == 0:
        print("Khong co cap so nao thoa man")
    else:
        print(f"So cap so co uoc chung lon nhat la 1 so nguyen to la: {len(gcd_primes)}")